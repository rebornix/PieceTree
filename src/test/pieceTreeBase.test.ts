...
/*
 * Ported from the VS Code piece tree test suite:
 ...
 * - the 'chunk based search' suite and the getNearestChunk test are omitted because
 *   findMatchesLineByLine / getNearestChunk are not part of this package
 */
...
describe('random is unsupervised', () => {
	it('splitting large change buffer', () => {
		const pieceTable = createTextBuffer([''], false);
		let str = '';
...
describe('snapshot', () => {
	it('bug #45564, piece tree pieces should be immutable', () => {
		const pieceTable = createTextBuffer(['\n']);
		pieceTable.insert(1, '!');
		const snapshot = pieceTable.createSnapshot('');
		const snapshot1 = pieceTable.createSnapshot('');
		assert.strictEqual(pieceTable.getLinesContent().join('\n'), getValueInSnapshot(snapshot));
	...
	it('immutable snapshot 3', () => {
		...
		assert.notStrictEqual(pieceTable.getLinesContent().join('\n'), getValueInSnapshot(snapshot));
	});

	it('snapshot with BOM', () => {
		const pieceTable = createTextBuffer(['abc\ndef']);
		assert.strictEqual(getValueInSnapshot(pieceTable.createSnapshot('\uFEFF')), '\uFEFFabc\ndef');
		assert.strictEqual(getValueInSnapshot(createTextBuffer(['']).createSnapshot('\uFEFF')), '\uFEFF');
	});
});
